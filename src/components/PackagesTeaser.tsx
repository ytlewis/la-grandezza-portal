  ...
  <section className="py-24 bg-black-soft">
    <div className="container mx-auto px-4 md:px-8">
      ...
      >
        ...
        <h2 className="font-heading text-3xl md:text-5xl text-cream mb-4">Our Packages</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {packages.map((pkg, i) => (
          <motion.div
            key={pkg.name}
            className={`relative p-8 text-center border ${pkg.accent} bg-black-deep/50 hover:shadow-gold transition-all duration-500 ${pkg.featured ? "lg:scale-105 shadow-gold-lg" : ""}`}
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          ...
          >
            ...
            <Link
              to="/packages"
              className="text-gold text-sm tracking-widest uppercase font-body hover:underline underline-offset-4"
            >
              View Details →
            </Link>
          </motion.div>
