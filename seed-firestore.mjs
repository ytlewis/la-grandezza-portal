// Seeds Firestore with default data using Firebase Auth
...
// ── Auth: sign in as admin to get write permission ─────────────────────────────
// Pass your admin email and password as args: node seed-firestore.mjs email password
const [,, adminEmail, adminPassword] = process.argv;
if (!adminEmail || !adminPassword) {
  console.error("Usage: node seed-firestore.mjs <admin-email> <admin-password>");
  process.exit(1);
}
...
async function seedCollection(colName, items) {
  ...
  if (!existing.empty) { console.log(`⏭  "${colName}" already has data, skipping`); return; }
...
const testimonials = [
  { id: "static-1", userId: "static", userName: "Margaret & Peter", userEmail: "", event: "Wedding", text: "La Grandezza transformed our wedding into a fairy tale. Every detail was perfect.", rating: 5, status: "approved", createdAt: "2026-01-15T00:00:00.000Z" },
  { id: "static-2", userId: "static", userName: "Safaricom Ltd.", userEmail: "", event: "Corporate Gala", text: "Professionalism at its finest. Our annual gala was executed flawlessly.", rating: 5, status: "approved", createdAt: "2026-02-20T00:00:00.000Z" },
  { id: "static-3", userId: "static", userName: "Angela Wanjiku", userEmail: "", event: "Birthday Celebration", text: "My 40th birthday was absolutely magical! The team went above and beyond.", rating: 5, status: "approved", createdAt: "2026-03-10T00:00:00.000Z" },
];
...
console.log("\n🎉 Done! Firestore is seeded and secured.");
process.exit(0);
